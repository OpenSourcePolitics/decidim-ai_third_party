  ...
  describe "#classify" do
    ...
    context "when the third-party request returns an invalid format" do
      ...
      it "raises an InvalidOutputFormat error" do
        expect do
          strategy.classify(content, organization_host,
                            klass)
        end.to raise_error(Decidim::Ai::SpamDetection::Openai::Strategy::InvalidOutputFormat)
      end
    end
    ...
    context "when the third-party request raises an error" do
      ...
      it "raises a ThirdPartyError" do
        expect do
          strategy.classify(content, organization_host,
                            klass)
        end.to raise_error(Decidim::Ai::SpamDetection::Openai::Strategy::ThirdPartyError)
      end
    end
  end
  ...
  describe "#parse_http_response" do
    ...
    context "when the response is forbidden" do
      ...
      it "raises a Forbidden error" do
        expect do
          strategy.parse_http_response(response)
        end.to raise_error(Decidim::Ai::SpamDetection::Openai::Strategy::Forbidden)
      end
    end
    ...
    context "when the response times out" do
      ...
      it "raises a TimeoutError" do
        expect do
          strategy.parse_http_response(response)
        end.to raise_error(Decidim::Ai::SpamDetection::Openai::Strategy::TimeoutError)
      end
    end
  end
